        ...
        if(treeData){
            return treeData.map((item) => {
                if (item.children && item.children.length > 0) {
                    ...
                    return <CheckboxItem className="refer-check-box" checked={false} onChange={(e) => this.onTreeChange(e, item)} key={item.id}>{item[this.props.displayField]}</CheckboxItem>;
                }
            });
        }
                        ...
                        >
                            <div style={modalHeight=='part'?{height:'93vh',width:'100vw'}:{height:'100vh',width:'100vw'}}>
                                ...
                                <NavBar leftContent="返回"
                                        key="nav"
                                        mode="light"
                                        onLeftClick={()=>{this.setState({selectedTreeId: null,showList:false});referTreeListParams={}}}
                                        rightContent={[
